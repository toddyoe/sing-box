...
func (r *Router) exchangeLoop(ctx context.Context, message *mDNS.Msg, options adapter.DNSQueryOptions, allowFakeIP bool) (*mDNS.Msg, adapter.DNSTransport, bool, error) {
	metadata := adapter.ContextFrom(ctx)
	var (
		response  *mDNS.Msg
		...
		if rule != nil {
			...
			case *R.RuleActionReject:
				...
				case C.RuleActionRejectMethodDrop:
					return nil, nil, false, tun.ErrDrop
				}
			case *R.RuleActionPredefined:
	...
	return response, transport, stale, err
}
...
func (r *Router) Exchange(ctx context.Context, message *mDNS.Msg, options adapter.DNSQueryOptions) (*mDNS.Msg, error) {
	...
	if options.Transport != nil {
		...
		response, transport, stale, err = r.exchangeLoop(ctx, message, options, true)
	}
	if err != nil {
		return nil, err
	}
...
func (r *Router) followPredefinedCNAME(ctx context.Context, message *mDNS.Msg, response *mDNS.Msg, options adapter.DNSQueryOptions) *mDNS.Msg {
	...
	followResp, _, followStale, _ := r.exchangeLoop(followCtx, followMsg, options, false)
	if followStale {
		r.logger.DebugContext(ctx, "updating stale cache ", FormatQuestion(followMsg.Question[0].String()))
		go r.Exchange(r.client.UpdateDnsCacheToContext(context.WithoutCancel(followCtx)), followMsg, options)
	}
